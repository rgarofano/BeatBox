...
static i2cDevice_t i2cDevices[] = {{"/dev/i2c-1", 0x70, "P9_17", "P9_18", 0}};
...
void I2C_writeByteToI2CReg(const int INDEX, unsigned char regAddr,
                           unsigned char value) {
...
void I2C_writeBytesToI2CReg(const int INDEX, unsigned char regAddr,
                            unsigned char values[], const int ARRAY_LENGTH) {
...
unsigned char I2C_readI2CReg(const int INDEX, unsigned char regAddr) {
  ...
  int bytesWritten =
      write(i2cDevices[INDEX].fileDesc, &regAddr, sizeof(regAddr));
