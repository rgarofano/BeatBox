/* Incomplete implementation of an audio mixer. Search for "REVISIT" to find
   things which are left as incomplete. Note: Generates low latency audio on
   BeagleBone Black; higher latency found on host.                           */
...
// Sample size note: This works for mono files because each sample ("frame') is
// 1 value. If using stereo files then a frame would be two samples.
...
// Client code must call AudioMixer_freeWaveFileData to free dynamically
// allocated data.
void AudioMixer_readWaveFileIntoMemory(char *fileName, wavedata_t *pSound) {
    ...
    int samplesRead =
        fread(pSound->pData, SAMPLE_SIZE, pSound->numSamples, file);
...
void AudioMixer_queueSound(wavedata_t *pSound) {
     ...
     *    processing the soundBites[] array, we must ensure access is
     * threadsafe.
     ...
     *    Note: You are only copying a pointer, not the entire data of the wave
     * file!
     * 4. After searching through all slots, if no free slot is found then print
     *    an error message to the console (and likely just return vs
     * asserting/exiting because the application most likely doesn't want to
     * crash just for not being able to play another wave file.
     */
    ...
    if (noEmptySlot) {
        printf("Error: No available space in the sound queue\n");
    }
}
...
void AudioMixer_cleanup(void) {
    ...
    //  in addition to this by calling AudioMixer_freeWaveFileData() on that
    //  struct.)
    free(playbackBuffer);
    playbackBuffer = NULL;
...
static void fillPlaybackBuffer(short *buff, int size) {
     ...
     * 3. Loop through each slot in soundBites[], which are sounds that are
     * either waiting to be played, or partially already played:
     *    - If the sound bite slot is unused, do nothing for this slot.
     *    - Otherwise "add" this sound bite's data to the play-back buffer
     *      (other sound bites needing to be played back will also add to the
     * same data).
     *      * Record that this portion of the sound bite has been played back by
     * incrementing the location inside the data where play-back currently is.
     *      * If you have now played back the entire sample, free the slot in
     * the soundBites[] array.
     *
     * Notes on "adding" PCM samples:
     * - PCM is stored as signed shorts (between SHRT_MIN and SHRT_MAX).
     * - When adding values, ensure there is not an overflow. Any values which
     * would greater than SHRT_MAX should be clipped to SHRT_MAX; likewise for
     * underflow.
     * - Don't overflow any arrays!
     * - Efficiency matters here! The compiler may do quite a bit for you, but
     * it doesn't hurt to keep it in mind. Here are some tips for efficiency and
     * readability:
     *   * If, for each pass of the loop which "adds" you need to change a value
     * inside a struct inside an array, it may be faster to first load the value
     * into a local variable, increment this variable as needed throughout the
     * loop, and then write it back into the struct inside the array after. For
     * example: int offset = myArray[someIdx].value; for (int i =...; i < ...;
     * i++) { offset ++;
     *           }
     *           myArray[someIdx].value = offset;
     *   * If you need a value in a number of places, try loading it into a
     * local variable int someNum = myArray[someIdx].value; if (someNum < X ||
     * someNum > Y || someNum != Z) { someNum = 42;
     *          }
    ...
    pthread_mutex_lock(&audioMutex);
    {
        for (int bufferIndex = 0; bufferIndex < size; bufferIndex++) {
            for (int i = 0; i < MAX_SOUND_BITES; i++) {
                wavedata_t *pCurrentSound = soundBites[i].pSound;
                if (pCurrentSound != NULL) {
                    int pDataIndex = soundBites[i].location;
                    int mergedAudioInt =
                        buff[bufferIndex] + pCurrentSound->pData[pDataIndex];

                    if (mergedAudioInt > SHRT_MAX) {
                        mergedAudioInt = SHRT_MAX;
                    } else if (mergedAudioInt < SHRT_MIN) {
